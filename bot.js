...
function dbCheckCooldownAndLock(userId, isOwner = false, cooldownSeconds = 15, dailyMax = 3) {
  const cleanId = String(userId).replace(/^tg:/i, '').trim();
    ...
    try {
      ...
      if (cdRow) {
        if (cdRow.active_scan_lock === 1) {
          // If lock is older than 2 minutes, consider it stale and allow
          if (now - cdRow.last_scan_time < 120000) {
            return { allowed: false, reason: 'SCAN_IN_PROGRESS' };
          }
        }

        if (!isVip && cdRow.cooldown_until && now < cdRow.cooldown_until) {
          const remaining = Math.ceil((cdRow.cooldown_until - now) / 1000);
      ...
      if (!isVip && currentCount >= dailyMax) {
        return { allowed: false, reason: 'QUOTA_EXCEEDED', remainingDailyScans: 0 };
      }
  ...
  if (cd.active_scan_lock === 1 && now - cd.last_scan_time < 120000) {
    return { allowed: false, reason: 'SCAN_IN_PROGRESS' };
  }
  if (!isVip && cd.cooldown_until && now < cd.cooldown_until) {
    return { allowed: false, reason: 'COOLDOWN', remainingSeconds: Math.ceil((cd.cooldown_until - now) / 1000) };
  }
...
function dbReleaseLockAndRecordScan(userId, record, cooldownSeconds = 15) {
  const cleanId = String(userId).replace(/^tg:/i, '').trim();
  const now = Date.now();
  const today = new Date().toISOString().split('T')[0];
...
function dbGetUserQuota(userId) {
  ...
  if (isVip) {
    return { used: 0, remaining: 9999, isVip: true };
  }
  ...
  if (!useFallbackDb && sqliteDb) {
    try {
      ...
      return { used, remaining: Math.max(0, 3 - used), isVip: false };
    } catch (e) {}
  }
  ...
  switch (command) {
    ...
    case '/quota': {
      ...
      return await sendTelegramMessage(
        ...
        `⚡ <i>made by @toshitzz</i>\n\n` +
        `• Scans Used Today: <b>${quota.used} / 3</b>\n` +
        `• Remaining Scans: <b>${quota.remaining}</b>\n` +
        `• Resets at: <b>00:00 UTC</b> (in <b>${getTimeUntilMidnightUtc()}</b>)\n\n` +
        `<i>Tip: The 3 scans/day limit ensures this service remains 100% free for everyone!</i>`,
        replyMsgId
      );
    }
    ...
    case '/myid': {
      return await sendTelegramMessage(
        ...
        `• User ID: <code>${cleanUserId}</code>\n` +
        `• Chat ID: <code>${chatId}</code>\n` +
        `• VIP Status: ${isOwner ? '👑 Active (Unlimited Scans)' : 'Standard (3 Scans/Day)'}\n\n` +
        `⚡ <i>RootGuard • made by @toshitzz</i>`,
        replyMsgId
      );
    }
  ...
  if (msg.document) {
    ...
    if (!lockCheck.allowed) {
      ...
      if (lockCheck.reason === 'COOLDOWN') {
        return await sendTelegramMessage(
          chatId,
          `⏳ <b>Cooldown Active:</b> Please wait <b>${lockCheck.remainingSeconds} seconds</b> before auditing another file.\n\n⚡ <i>made by @toshitzz</i>`,
          msg.message_id
        );
      }
      if (lockCheck.reason === 'QUOTA_EXCEEDED') {
        return await sendTelegramMessage(
          chatId,
          `⛔ <b>Daily Scan Limit Reached (3/3 scans)</b>\n` +
          `⚡ <i>made by @toshitzz</i>\n\n` +
          `To keep this service 100% free with AI auditing, each standard user receives 3 scans per day.\n\n` +
    ...
    const res = await callTelegram(
      'setMyCommands',
      {
        commands: [
          ...
          { command: 'myid', description: '🆔 Your Telegram ID & VIP Status' },
